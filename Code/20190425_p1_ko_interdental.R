...
ko <- SetIdent(ko, cells.use = WhichCells(ko, ident = idents[-1]), ident.use = "AOC")
...
object <- SubsetData(ko, ident.use = c( "Interdental", "AOC"))
...
saveRDS(results, "/data/kollal2/desingle/20190425_ko_interdental_aoc_results.Rds")

results.classified <- DEtype(results = results, threshold = 0.05)
saveRDS(results.classified, "/data/kollal2/desingle/20190425_ko_interdental_aoc_results_classified.Rds")

results.sig <- results.classified[results.classified$pvalue.adj.FDR < 0.05, ]
saveRDS(results.sig, "/data/kollal2/desingle/20190425_ko_interdental_aoc_results_sig.Rds")
