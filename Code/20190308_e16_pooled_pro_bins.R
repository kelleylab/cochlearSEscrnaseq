...
E16 <- readRDS("/data/kollal2/scenic/e16_epi_final.Rds")
E16 <- SetIdent(E16, cells.use = WhichCells(E16, ident = c("Lateral Prosensory", "Medial_Prosensory")),
                ident.use = "Prosensory")


E16@meta.data$clus.ident <- E16@ident
timepoint <- as.data.frame(as.matrix(E16@meta.data$timepoint))
clus.ident <- as.data.frame(as.matrix(E16@meta.data$clus.ident))
stim <- as.data.frame(as.matrix(E16@meta.data$stim))
metadata <- cbind(timepoint, clus.ident, stim)
colnames(metadata) <- c("timepoint", "ident", "batch")
rownames(metadata) <- colnames(E16@data) 


#get the raw counts data and metadata from Seurat

raw.data <- as.matrix(E16@raw.data)
data <- raw.data[rownames(raw.data) %in% rownames(as.matrix(E16@data)),
                 colnames(raw.data) %in% colnames(as.matrix(E16@data))]
data <- data[rownames(E16@data), colnames(E16@data)]
...
pooled_metadata <- cbind(pooled_metadata, rep("E16", length(pooled_metadata)))
colnames(pooled_metadata) <- c("ident","timepoint")
rownames(pooled_metadata) <- colnames(pooled_data)

###RUNNING SCENIC:
##create directory 
dir.create("/data/kollal2/scenic/20190308_e16_bins")
setwd("/data/kollal2/scenic/20190308_e16_bins")
dir.create("int")
...
myDatasetTitle="Pooled E16 20190308 scenic" # choose a name for your analysis
