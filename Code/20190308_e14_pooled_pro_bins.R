...
E14 <- readRDS("/data/kollal2/scenic/e14_epi_final.Rds")
E14 <- SetIdent(E14, cells.use = WhichCells(E14, ident = c("Lateral Prosensory", "Medial_Prosensory")),
               ident.use = "Prosensory")


E14@meta.data$clus.ident <- E14@ident
E14@meta.data$timepoint <- E14@ident
timepoint <- as.data.frame(as.matrix(E14@meta.data$timepoint))
clus.ident <- as.data.frame(as.matrix(E14@meta.data$clus.ident))
stim <- as.data.frame(as.matrix(E14@meta.data$stim))
metadata <- cbind(timepoint, clus.ident, stim)
colnames(metadata) <- c("timepoint", "ident", "batch")
rownames(metadata) <- colnames(E14@data) 


#get the raw counts data and metadata from Seurat

raw.data <- as.matrix(E14@raw.data)
data <- raw.data[rownames(raw.data) %in% rownames(as.matrix(E14@data)),
                 colnames(raw.data) %in% colnames(as.matrix(E14@data))]
data <- data[rownames(E14@data), colnames(E14@data)]
...
pooled_metadata <- cbind(pooled_metadata, rep("E14", length(pooled_metadata)))
colnames(pooled_metadata) <- c("ident","timepoint")
rownames(pooled_metadata) <- colnames(pooled_data)

###RUNNING SCENIC:
##create directory 
dir.create("/data/kollal2/scenic/20190308_e14_bins")
setwd("/data/kollal2/scenic/20190308_e14_bins")
dir.create("int")
...
myDatasetTitle="Pooled E14 20190304 scenic" # choose a name for your analysis
